#modulo de Web Scraper dinámico o ingesta datos de pruebas de +1200km en Les BRM 1200+ disponibles + mucha otra
#https://www.randonneur.me/ nota es una web SPA (Single Page Application),
# pedir al servidor el archivo JSON original de resultados ERROR,
# con BeautifulSoup
#para conectar a web real, extraerá los resultados históricos, datos PUBLICOS
# los guardará en la bbdd SQLite local conectando las tablas de forma auto
#historial y cambios:
#Intento 1 (BeautifulSoup / HTML Scraping): Intentaba extraer la tabla
  #directamente del HTML de la pagina principal. Fallo debido a que la web es una
...
#Intento 4 (Fusiòn Relacional Integrada en Memoria): Se diseña una solucion ETL
...
import re
import json  #  json para procesar el texto limpio
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Se limita la ingesta inicial a los primeros 100 resultados de la historia para pruebas
        pruebas_a_procesar = results_data[:100]
        ...
        for r_item in pruebas_a_procesar:
            ...
            nombre = user_data.get('fname', '').strip().title()
            apellidos = user_data.get('lname', '').strip().upper()
            raw_sex = user_data.get('sex', '').strip().upper()  # Se captura el genero para estadistica
            ...
            # Se crea o recupera el resultado de participacion
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f" - {eventos_creados} nuevos Eventos LRM indexados.\n"
...
#EJECUTAR en terminal despues de guardar:
